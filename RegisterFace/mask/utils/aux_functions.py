...
from mask.utils.read_cfg import read_cfg
from mask.utils.fit_ellipse import *
import random
from mask.utils.create_mask import texture_the_mask, color_the_mask
...
def mask_face(image, face_location, six_points, angle, type="surgical", pattern=None, pattern_weight=None, color=None,
              color_weight=None):
    debug = False
    ...
    if not "empty" in type and not "inpaint" in type:
        cfg = read_cfg(config_filename=os.getcwd()+"/mask/masks/masks.cfg",
                       mask_type=type, verbose=False)
    else:
        if "left" in type:
...
def mask_image(image, mask_type, verbose, detector, predictor, pattern, color, code=None, mask_dict_of_dict=None,
               code_count=None, pattern_weight=None, color_weight=None):
    # Read the image
    ...
    verbose = verbose
    if code:
        ind = random.randint(0, len(code_count) - 1)
        mask_dict = mask_dict_of_dict[ind]
        mask_type = mask_dict["type"]
        color = mask_dict["color"]
        pattern = mask_dict["texture"]
        code_count[ind] += 1
    ...
    for (i, face_location) in enumerate(face_locations):
        ...
        if mask_type != "all":
            ...
            image, mask_binary = mask_face(
                image, face_location, six_points_on_face, angle, type=mask_type, pattern=pattern,
                pattern_weight=pattern_weight, color=color, color_weight=color_weight
...
def get_available_mask_types(config_filename=os.getcwd()+"/mask/masks/masks.cfg"):
    ...
    for line in lines:
        ...
        if line.startswith("[") and line.endswith("]"):
            ...
            if "left" not in current_section and "right" not in current_section:
                available_mask_types.append(current_section)
